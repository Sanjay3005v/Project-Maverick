...
const EvaluateCodeChallengeOutputSchema = z.object({
  ...
  feedback: z.string().describe("A brief explanation of why the code passed or failed. Provide a hint if it failed, and include feedback on code quality and best practices."),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert code evaluator and a helpful coding coach. A user has submitted a solution for a coding challenge.
Your task is to analyze if the code correctly and logically solves the problem, and also to assess its quality.
...
4.  Assess the code's quality, considering factors like clarity, efficiency, and adherence to language-specific best practices.
5.  Determine if the solution is correct.
...
- If the code correctly solves the problem, set the status to "Passed". Your feedback should be positive and also include a brief comment on the code's quality or a suggestion for a best practice.
- If the code is incorrect, incomplete, or has logical errors, set the status to "Failed". Provide a concise, helpful hint that is directly related to the user's code and the problem. Do not give away the full answer. Add a constructive comment on code style or structure if relevant.
  `,
});
